...
class Node{
    public:
        ...
        Node* next; //Used for header Table to find the node with same item.

        Node(int item, int count, Node* parent){
            this->item = item;
            this->count = count;
            this->parent = parent;
            this->next = nullptr;
...
using HeaderTable = unordered_map< int, pair< Node*, int> >;

//HeaderTable : item, node*, total support
...
void constructTree(HeaderTable& headerTable,Node* root, vector<vector<int> >& transactions, vector<int>& supportTable){
    ...
    for(auto &transaction : transactions){
        ...
        for (auto i : transaction){
            ...
            bool flag = false; //Suppose the item is not in the tree.

            for(auto ch : cur->children){//for each item in current node's children vector
                if(ch->item != i) continue;
                ch->count++; //if the item is found, then increase the count.        
                cur = ch;
                flag = true;
                break;
            }
...
void show(string prefix, Node* root){
    ...
    cout << prefix << root->item << ':' << root->count << endl;
